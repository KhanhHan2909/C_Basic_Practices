...
void eulerpath(int a[100][100],int n,int *v,int *t,bool *done)
{
	int loopbreak = 0, i, j, count = 0, marked = -1, current_v = v[*t], current_d = deg(a,n,current_v);
	...
	while(true)
	{
		...
		if(isconnected) eulerpath(a,n,v,t,done);
		else
		{
			int donetest = 0;
		...
		if(current_d<2||*done) break;
		if(current_d>1&&count==current_d)
		{
			edgereturn(a,n,v,t);
			break;
		}
...
{
	int zero = 0;
	int i, j, a[100][100], n, start, evencount = 0, oddcount = 0, *v, *t = &zero;
	...
	if(oddcount==2&&evencount==n-2)
	{
		v[0] = start;
		eulerpath(a,n,v,t,done);
		print(v,t);
	}
	else printf("\nDo thi da cho khong co duong di Euler.");
	free(v);
}